...
export function DevDashboard() {
  ...
  useEffect(() => {
    const fetchSystemInfo = async () => {
      try {
        ...
        // Mock build info - would come from a real API in production
        setBuildInfo({
        ...
        setError(err.message || 'Error fetching system information');
        console.error(err);
      } finally {
        setIsLoading(false);
      }
  ...
  if (isLoading) {
    return <div className={styles.dashboard}><p>Loading system information...</p></div>;
  }
    ...
    <div className={styles.dashboard}>
              ...
              <div className={styles.progressContainer}>
                <div 
                  ...
                  style={{ 
                    ...
                    backgroundColor: systemInfo.cpu.usage > 80 ? '#ff4d4f' : '#1890ff' 
                  }}
                />
              </div>
      ...
      <div className={styles.section}>
        ...
        <div className={styles.buttonGrid}>
          <button className={styles.button} onClick={() => alert('Clearing cache...')}>
            Clear Cache
          </button>
          <button className={styles.button} onClick={() => alert('Running tests...')}>
            Run Tests
          </button>
          <button className={styles.button} onClick={() => alert('Generating sitemap...')}>
            Generate Sitemap
          </button>
          <button className={styles.button} onClick={() => alert('Running lint...')}>
            Lint Code
          </button>
        </div>
      </div>
